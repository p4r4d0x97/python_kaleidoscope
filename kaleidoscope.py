...
def load_xml(path):
    ...
    for device in root.findall('device'):
        entry = {
            ...
            # 'new_field': device.find('new_field').text if device.find('new_field') is not None else '',
        }
        data.append(entry)
...
# unique_new_fields = sorted(df['new_field'].unique())
# new_field_colors = {nf: random_color() for nf in unique_new_fields}  For unique color
...
def get_histogram_data():
    ...
    # new_field_counts = df['new_field'].value_counts().reset_index()  _ histogram for new entry
    # new_field_counts.columns = ['new_field', 'count']
    return vlan_counts, switch_counts, criticality_counts  # new_field_counts  - Add for histogram
...
dashboard_layout = html.Div([
    ...
    html.Div([
        ...
        # dcc.Dropdown(id='filter-new-field', options=[{'label': f, 'value': f} for f in sorted(df['new_field'].unique())], - Add here for dropdown menu
        #             placeholder='Select New Field',multi=True,style={'width': '15%'})
        ...
        children=[
            dash_table.DataTable(
                ...
                columns=[
                    ...
                    # {"name": "New Field", "id": "new_field"}, Add to dashboard
                ],
                data=[],
...
@app.callback(
    ...
    # Input('filter-new-field', 'value'), Add for a filter
)
# Add here new for dashboard in  upadte_table function
    ...
    if pathname and pathname.startswith('/device/'):
        ...
        try:
            ...
            details = [
                ...
                # html.P(f"New Field: {device['new_field']}"),
            ]
...
def display_config_content(view_n_clicks, explorer_n_clicks, html_n_clicks, view_button_ids, explorer_button_ids,
    ...
    if not ctx.triggered or (not any(view_n_clicks) and not any(explorer_n_clicks) and html_n_clicks == 0):
        print("No button clicked or no clicks registered.")
        return ""

    try:
        triggered_id = ctx.triggered[0]['prop_id'].split('.')[0]
        if triggered_id == 'open-html-btn':
